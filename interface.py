...
class Interface:
    ...
    def bfs(self, start_node, last_node):
        ...
        with self._driver.session() as session:
            ...
            result = session.run(
                "MATCH (start) WHERE start.name = $start_node_name RETURN id(start) AS start_node_id",
            ...
            result = session.run(
                "MATCH (end) WHERE end.name = $end_node_name RETURN id(end) AS end_node_id",
                end_node_name=last_node
            ).single()
            end_node_id = result["end_node_id"] if result else None
            if start_node_id is None and end_node_id is None:
                print("Its all none!!!")
