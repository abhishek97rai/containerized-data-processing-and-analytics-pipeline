...
class DataLoader:
    ...
    def load_transform_file(self, file_path):
        ...
        # TODO: Your code here
        # Load the CSV file into Neo4j according to the provided schema
        with self.driver.session() as session:
            ...
            CREATE (pickup)-[:TRIP {distance: toFloat(line.trip_distance), fare: toFloat(line.fare_amount),
                    pickup_dt: datetime(REPLACE(line.tpep_pickup_datetime, ' ', 'T')).epochSeconds,
                    dropoff_dt: datetime(REPLACE(line.tpep_dropoff_datetime, ' ', 'T')).epochSeconds}]->(dropoff)
            """
            session.run(query)
